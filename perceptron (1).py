...
# SPECIFICATION: training perceptron and MLP to output accuracy after using optdigist.tes file
...
for w in n: #iterates over n
    ...
    for b in r: #iterates over r
        ...
        for a in range(2): #iterates over the algorithms
            ...
            if a == 0:
                   if counter == 0:
                          highestAccuracyPer = accuracy
                   else:
                          if accuracy > highestAccuracyPer:
                                 highestAccuracyPer = accuracy
                   print("Highest Perceptron accuracy so far: " , highestAccuracyPer , ", Parameters: learning rate=" ,w , ", shuffle=" , b)
            else:
                   if counter == 0:
                          highestAccuracyMLP = accuracy
                   else:
                          if accuracy > highestAccuracyMLP:
                                 highestAccuracyMLP = accuracy
                   print("Highest MLP accuracy so far: " , highestAccuracyMLP , ", Parameters: learning rate=", w, ", shuffle=", b) 
